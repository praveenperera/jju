...
pub fn run_with_flags(flags: Jj) -> Result<()> {
    match flags.subcommand {
        ...
        Some(JjCmd::Clean) => clean(),
        Some(JjCmd::RebaseOnto { revision, update }) => rebase_onto(&revision, update),
...
fn rebase_onto(revision: &str, update: bool) -> Result<()> {
    ...
    if update {
        ...
        cmd!("jj", "bookmark", "set", &trunk, "-r", revision)
            .stdout_null()
            .stderr_null()
            .run()
...
fn preview_hunks(revision: &str, file_filter: Option<&str>) -> Result<()> {
    let diff_output = if let Some(file) = file_filter {
        cmd!("jj", "diff", "-r", revision, "--git", file)
            .stdout_capture()
            .stderr_capture()
            .read()
...
fn split_hunk(
    ...
    // preview mode
    if preview {
        return preview_hunks(revision, file_filter.as_deref());
    }
    ...
    cmd!("jj", "new", &parent_rev, "-m", &message)
        .stdout_null()
        .stderr_null()
        .run()
    ...
    for (file_idx, hunk_indices) in &hunks_by_file {
        ...
        let parent_content = if file.is_new {
            ...
            cmd!("jj", "file", "show", "-r", &parent_rev, file_path)
                .stdout_capture()
                .stderr_capture()
                .read()
                .unwrap_or_default()
    ...
    for (file_idx, selected_hunk_indices) in &hunks_by_file {
        ...
        let _full_new_content = if file.is_new {
            cmd!("jj", "file", "show", "-r", revision, file_path)
                .stdout_capture()
                .stderr_capture()
                .read()
                .unwrap_or_default()
